...
% Random varaibles distributions    
    distrR=1;
    distrS=1; %Distribution type (1 = Normal;2 = Lognormal;3 = Gamma;4 = Shifted Exponential marginal distribution;  5 = Shifted Rayleigh marginal distribution; 6 = Uniform distribution; 7 = Beta; 8 = Chi-square; 11 = Type I Largest Value marginal distribution;12 = Type I Smallest Value marginal distribution                                                             %
...
for VR=[.05 .15 .30] % Values of the Resistance Coefficient of Variation
    ...
    for VS=[.10 .30 .45 .60] % Values of the load coeff. of variation
       % Find the optimal Pf and optimal design for different combinations of H and CI
            clear H CI
            [H,CI]=meshgrid(dum_H,[logspace(-5,-1,res)]);
            for c_h=1:size(H,2)
       ...
       % Plot ln([(C0+CI p_opt+H) * (i_plot+omega_plot) ] / [CI * (i+omega)] )  VS. beta_opt
            dum_log_rho_div_CI=[log(((C0+CI(1,:).*p_opt(1,:)+H(1,:))./C0)./(CI(1,:)./C0))];
            dum_Pfopt=[Pf_opt(1,:)];
            for k=2:res
               dum_log_rho_div_CI=[dum_log_rho_div_CI log(((C0+CI(k,:).*p_opt(k,:)+H(k,:))./C0)./(CI(k,:)./C0))];
...
xlabel(strcat('ln([(C_0+C_I p_o_p_t+H) \cdot ',num2str(0.01*round(og*100)),' ] / [C_I \cdot ({i^{(1a)}+\omega})] )  \cong ln([(C_0+H) \cdot 0.04] / [C_I \cdot ({i^{(1a)}+\omega})] )'));
ylabel('\beta_{opt}^{(1a )}')
